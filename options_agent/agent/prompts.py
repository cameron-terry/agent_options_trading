...
def build_system_prompt(playbook: PlaybookConfig, limits: Limits) -> str:
    ...
    return (
        ...
        "\n"
        "**5. covered_call / cash_secured_put conditions**\n"
        "- covered_call: only propose if get_portfolio_state() confirms "
        "≥ 100 shares of the underlying per contract you intend to sell.\n"
        "- cash_secured_put: only propose if buying power supports full cash "
        "reservation at put_strike × 100 × contracts.\n"
        "\n"
        ...
        "\n"
        "2. **get_events(symbol)** — identify earnings, ex-dividend dates, "
        "and macro events within the target expiration window.\n"
        "\n"
        "3. **Directional thesis** — use price, technicals, and regime "
        "context to form a directional bias (bullish / bearish / neutral).\n"
        "\n"
